...
# 修改版 PromptClassifier：新增 return_prompt_details=True 时回传每类 per-prompt logits 与 best idx
class PromptClassifier(nn.Module):
    ...
    def forward(self, pixel_values=None, prompt_inputs=None,
                return_prompt_details: bool = False, **kwargs):
        '''
        pixel_values: image tensor after transform
        prompt_inputs: dict like {'classA': {'input_ids':..., 'attention_mask':...}, ...}
        return_prompt_details: if True, also return per-class per-prompt logits and best idx
        '''
        pixel_values = pixel_values.cuda()
        class_similarities = []
        class_names = []

        # 新增：收集每个类的 per-prompt logits（与 class_names 同序）
        per_class_prompt_logits = []   # List[Tensor [num_prompts]]
        per_class_best_idx = []        # List[int]
        ...
        for cls_name, cls_text in prompt_inputs.items():
            ...
            logits = medclip_outputs['logits']      # 预期 [N_img, N_prompts]，此处 N_img=1

            # 兼容：把 prompt 维拉出来，得到 [N_prompts]
            if logits.ndim == 2 and logits.shape[0] == 1:
                prompt_logits = logits.squeeze(0)   # [N_prompts]
            else:
                # 回退策略：确保 prompt 维在最后
                prompt_logits = logits.reshape(1, -1).squeeze(0)

            # 类相似度（与原逻辑保持一致）
            if self.ensemble:
                cls_sim = torch.mean(logits, dim=1)  # [N_img] -> [1]
            else:
                cls_sim = torch.max(logits, dim=1)[0]  # [N_img] -> [1]

            class_similarities.append(cls_sim)  # [1]
            class_names.append(cls_name)

            if return_prompt_details:
                per_class_prompt_logits.append(prompt_logits.detach().float().cpu())  # [N_prompts]
                per_class_best_idx.append(int(torch.argmax(prompt_logits).item()))

        class_similarities = torch.stack(class_similarities, dim=1)  # [1, N_class]
        outputs = {
            'logits': class_similarities,
            'class_names': class_names,
        }
        if return_prompt_details:
            outputs['per_class_prompt_logits'] = per_class_prompt_logits
            outputs['per_class_best_idx'] = per_class_best_idx
        return outputs
