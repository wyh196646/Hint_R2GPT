...
#     return prompts
# medclip/prompts.py 里覆盖同名函数（或放到你工程里确保被导入）

def generate_chexpert_class_prompts(n=None, return_map=False, seed=None):
    """
    Generate text prompts for each CheXpert classification task.
    ...
    ----------
    n : int or None
        number of prompts per class (random sample). If None, use all.
    return_map : bool
        if True, also return a dict mapping each prompt text to its (severity, subtype, location):
        {cls: {prompt_text: (sev, sub, loc)}}
    seed : int or None
        random seed for reproducibility when n is not None.
    ...
    if return_map is False:
        ...
        (prompts, prompt2triple_by_class)
    """
    rng = random.Random(seed)
    prompts = {}
    prompt2triple_by_class = {}

    for cls_name, node in constants.MIMIC_CLASS_PROMPTS.items():
        # 通常 keys 顺序为 ["severity","subtype","location"]
        keys = list(node.keys())
        severities = node.get(keys[0], [""])
        subtypes   = node.get(keys[1], [""])
        locations  = node.get(keys[2], [""])

        all_texts, all_triples = [], []
        ...
        for sv in severities:
            for st in subtypes:
                for loc in locations:
                    ...
                    all_triples.append((sv, st, loc))

        if n is not None and n < len(all_texts):
            idxs = rng.sample(range(len(all_texts)), n)
            texts = [all_texts[i] for i in idxs]
            triples = [all_triples[i] for i in idxs]
        else:
            texts = all_texts
            triples = all_triples

        prompts[cls_name] = texts
        if return_map:
            prompt2triple_by_class[cls_name] = {t: tri for t, tri in zip(texts, triples)}

        #print(f"sample {len(texts)} num of prompts for {cls_name} from total {len(all_texts)}")

    if return_map:
        return prompts, prompt2triple_by_class
    return prompts
